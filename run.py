...
def validate_data(values):
    ...
    Rases ValueError if strings cannot be converted into integers or if there
    are not exactly 6 values.
    """
    try:
...
def update_worksheet(data, worksheet):
    """
    Update stock and surplus worksheets, adding new row with the list data
    provided
    """
    print(f"Updating {worksheet} worksheet...\n")
    work_sheet_to_update = SHEET.worksheet(worksheet)
    work_sheet_to_update.append_row(data)
...
def calculate_stock_data(data):
    ...
    for column in data:
        ...
        new_stock.append(round(average * 1.1))

    return new_stock


def get_stock_values(data):
    headings = [key for key in SHEET.worksheet("stock").get_all_values()[0]]
    my_dict = {}

    for heading, value in zip(headings, data):
        my_dict[heading] = value
